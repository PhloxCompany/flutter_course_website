...
class ProfileMobileModeProvider extends ChangeNotifier {
  ...
  requestProfileImage(BuildContext context, Uint8List file) async {
    await ApiService(context).post(
        url: 'update_profile.php?api=profile_image',
        res: (response) {
          if (response != false) {
  ...
  requestEditProfile(BuildContext context) async {
    await ApiService(context).post(
        url: 'update_profile.php?api=profile_details',
        res: (response) {
          if (response != false) {
            Navigator.pushNamedAndRemoveUntil(
                context, HomePage.routeName, (route) => false);
  ...
  PhloxAnimationsController profileAnimationItemMenu =
      PhloxAnimationsController();
  PhloxAnimationsController profileAnimationText = PhloxAnimationsController();
  PhloxAnimationsController profileAnimationExpanded =
      PhloxAnimationsController();
